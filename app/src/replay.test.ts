...
describe("buildTimeline", () => {
  test("orders by ts, then id as tiebreak", () => {
    const steps = buildTimeline([
      feature({ id: "s:3", ts: 300 }),
      feature({ id: "s:1", ts: 100 }),
      feature({ id: "s:5", ts: 100 }),
      feature({ id: "s:2", ts: 200 }),
    ]);
...
describe("frameAt", () => {
  ...
  test("g=0 starts the first route un-drawn", () => {
    expect(frameAt(timeline, 0)).toEqual({ index: 0, id: "s:1", ts: 100, trim: 0 });
  });
  ...
  test("mid-slice yields the fractional trim within the active route", () => {
    ...
    expect(frameAt(timeline, 0.375)).toEqual({ index: 1, id: "s:2", ts: 200, trim: 0.5 });
  });

  test("g>=1 draws the last route fully", () => {
    expect(frameAt(timeline, 1)).toEqual({ index: 3, id: "s:4", ts: 400, trim: 1 });
    expect(frameAt(timeline, 2)).toEqual({ index: 3, id: "s:4", ts: 400, trim: 1 });
  });

  test("clamps negative progress to the start", () => {
    expect(frameAt(timeline, -1)).toEqual({ index: 0, id: "s:1", ts: 100, trim: 0 });
  });
});
